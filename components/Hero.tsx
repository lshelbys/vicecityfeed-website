        ...
        <Link href={href} className="block">
          <div className="relative aspect-16/10 min-h-[22rem] overflow-hidden md:min-h-[28rem] lg:min-h-[34rem]">
            <CoverArt
              ...
              title={article.title}
              className="absolute inset-0 h-full w-full origin-center transition-transform duration-300 group-hover:scale-105"
            />
            <div className="absolute inset-0 bg-linear-to-t from-black via-black/50 to-transparent" />
            <div className="absolute inset-x-0 bottom-0 flex flex-col items-start gap-4 p-6 md:p-10">
              <span className="rounded-full bg-black/70 px-3 py-1 text-[11px] font-semibold tracking-wide text-white uppercase">
                {CATEGORY_SHORT[article.category]}
              </span>
              <h1
                id="hero-heading"
                className="max-w-3xl text-3xl font-extrabold tracking-tight text-paper md:text-5xl lg:text-6xl"
              >
                {article.title}
              </h1>
              <span className={ctaPillClass("white")}>{cta}</span>
            </div>
          </div>
        </Link>
      </article>
